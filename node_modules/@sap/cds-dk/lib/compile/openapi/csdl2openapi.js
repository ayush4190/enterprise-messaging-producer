...
module.exports.csdl2openapi = function (
    ...
    const openapi = {
        ...
        servers: getServers(serviceRoot),
        tags: getTags(entityContainer),
        paths: getPaths(entityContainer),
            ...
            Object.keys(schema.$Annotations || {}).forEach(target => {
                ...
                if (open == -1) {
                    ...
                    element = element.find(overload =>
                        ...
                        || (overload.$Parameter || []).map(p => { const type = p.$Type || 'Edm.String'; return p.$Collection ? `Collection(${type})` : type }).join(',') == args);
                }
                if (!element) {
    ...
    function getInfo(csdl, entityContainer) {
        ...
        ) + (diagram ? getResourceDiagram(csdl, entityContainer) : '');
        return {
     ...
     */
    function getServers(serviceRoot) {
    ...
    function operationCreate(pathItem, element, name, sourceName, targetName, target, level, restrictions) {
        ...
        if (insertRestrictions.Insertable !== false) {
            ...
            pathItem.post = {
                ...
                responses: response(201, 'Created ' + lname, { $Type: element.$Type })
            };
    ...
    */
    ...
    function schemasForStructuredType(schemas, qualifier, name, type) {
        ...
        Object.keys(properties).forEach(iName => {
            ...
            allProperties[iName] = getSchema(property);
            if (property.$Kind == 'NavigationProperty') {
                if (property.$Collection) {
                    allProperties[`${iName}@${csdl.$Version === '4.0' ? 'odata.' : ''}count`] = ref('count');
                }
                creProperties[iName] = getSchema(property, '-create');
                if (deepUpdate)
                    updProperties[iName] = getSchema(property, '-create');
            } else {
                if (property[voc.Core.Permissions] == 'Read' || property[voc.Core.Computed]) {
                    ...
                    creProperties[iName] = getSchema(property, '-create');
                    if (!isKey[iName] && !property[voc.Core.Immutable])
                        updProperties[iName] = getSchema(property, '-update');
                }
            }
        });
...
module.exports.deleteUnreferencedSchemas = function (openapi) {
    ...
    do {
        gReferenced = {};
        getReferencedSchemas(openapi, gReferenced);
        gDeleted = deleteUnreferenced(openapi.components.schemas, gReferenced, '#/components/schemas/');
    } while (gDeleted)

    deleteUnreferenced(openapi.components.parameters, gReferenced, '#/components/parameters/');
