module.exports = Object.assign(deploy, {
  options: ['--to', '--tunnel-address', '--vcap-file'], flags: [ '--no-save', '--auto-undeploy', '--dry', '--with-mocks', '--bind' ],
  shortcuts: ['-2'],
  help: `
# SYNOPSIS
...
# OPTIONS
    ...
    *--bind* (beta feature, only in combination with *--to hana*)

      Use *cds bind* to connect the created service after deployment.
      This will no longer create default-env.json files. Instead, cds bind is used to
      create and store connection information in file *.cdsrc-private.json*.
...
async function updata_package_json (kind, model, database) {
  ...
  else if (kind === 'sqlite') requires.db.credentials = { database:'sqlite.db' }
